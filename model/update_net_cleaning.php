...
if ($_SESSION['role'] === 'admin') {
    ...
    $current = $conn->query("SELECT assigned_to FROM net_cleaning WHERE id = $id")->fetch_assoc();
...
if ($stmt->execute()) {
    ...
    if ($_SESSION['role'] !== 'admin' && $status === "Completed") {
        ...
        if ($admin) {
            $admin_id = $admin['id'];
            $message = $_SESSION['name'] . " completed a Net Cleaning task.";
            $notifStmt = $conn->prepare("
                INSERT INTO notifications (user_id, title, message, is_read, created_at)
    ...
    $_SESSION['popup_success'] = "Net Cleaning updated successfully.";
