...
if ($_SESSION['role'] === 'admin') {
    ...
    $current = $conn->query("SELECT assigned_to FROM net_repairing WHERE id = $id")->fetch_assoc();
    $old_assigned = $current['assigned_to'];

    $stmt = $conn->prepare("
        UPDATE net_repairing
...
if ($stmt->execute()) {
    ...
    if ($_SESSION['role'] === 'admin' && $assigned_to && $assigned_to != $old_assigned) {
        $message = "You have been assigned a new Net Repairing task.";
    ...
    if ($_SESSION['role'] !== 'admin' && $status === "Completed") {
        ...
        if ($admin) {
            $admin_id = $admin['id'];
            $message = $_SESSION['name'] . " completed a Net Repairing task.";
            $notifStmt = $conn->prepare("
                INSERT INTO notifications (user_id, title, message, is_read, created_at)
    ...
    $_SESSION['popup_success'] = "Net Repairing updated successfully.";
